...
TEST_CASE("jobs run views in order and preserve insertion order for ties") {
    ...
    registry.job<Position>(10).each([&](ashiato::Entity current, Position&) {
        REQUIRE(current == entity);
        calls.push_back(10);
    });
    registry.job<Position>(-1).each([&](ashiato::Entity current, Position&) {
        REQUIRE(current == entity);
        calls.push_back(-1);
    });
    registry.job<Position>(10).each([&](ashiato::Entity current, Position&) {
...
TEST_CASE("run jobs can exclude jobs by job entity tag") {
    ...
    const ashiato::Entity disabled = registry.component<Disabled>();
...
TEST_CASE("jobs can filter iterated entities by typed tags") {
    ...
    registry.job<Position>(0)
        ...
        .each([](auto& view, ashiato::Entity, Position& position) {
            position.x += 10;
...
TEST_CASE("mutable job views mark iterated components dirty") {
    ...
    registry.job<Position>(0).each([&](ashiato::Entity current, Position& position) {
        REQUIRE(current == entity);
